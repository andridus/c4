defmodule C4.Dom do
  ...
  def dom({node, attrs_, children_}, assigns) when is_bitstring(node) and is_list(children_) do
    "<#{node} #{attributes(attrs_)} >"<>Enum.map_join(children_, &(dom(&1, assigns)))<>"</#{node}>"
  ...
  defp parse_component({node, attrs_, children_}, assigns) do
    case node do
      :label -> dom_label(attrs_, children_, assigns)
      :input -> 
      ...
      atom -> "<.live_component #{attributes(attrs_)} module={#{atom}} id=\"#{C4.Utils.unique(10)}\" />"
    end
  end
  ...
  defp dom_label(attrs, children, assigns) do
    case children do
      ...
      chld -> {"label", [ class: attrs[:class], style: attrs[:style]], chld}
    end
  end
end